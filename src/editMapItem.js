...
const EditMapItem = function (sessionKey) {
  ...
  (this.Init = () => {
    ...
    (this.UploadImg = (jQbtn, FinishedFn) => {
      ...
      imageUpload.Init();
      // eslint-disable-next-line no-undef
      imageUpload.Upload(jQbtn, methodName, pam, function (_file, response) {
        try {
    ...
    (this.SetMapItemInfo = () => {
      ...
      for (let i in itemlist) {
        ...
        itemlist[i].itemImageUrl = imgInfo ? imgInfo.mapItemImageUrl : null;
        ...
        switch (parseInt(itemlist[i].typeId)) {
          case 1:
            ...
            for (let index in self.CameraList) {
              if (
                self.CameraList[index].cameraId == itemlist[i].typePrimaryId
              ) {
            ...
            if (!obj) {
              ...
              //Set GroupName
              let group = self.cameraGroupList.GetGroupByID(obj.cameraGroupId);
              if (group != null) {
          ...
          case 2:
            ...
            break;
          case 3:
            obj = self.mapList.GetMapByID(itemlist[i].typePrimaryId);
            if (obj == null) {
              delete itemlist[i];
            } else {
            ...
            break;
          case 4:
            obj = self.commandList.GetCommandByID(itemlist[i].typePrimaryId);
            if (obj == null) {
              delete itemlist[i];
            } else {
              const nameList = {
      ...
      MapInfoRequest.Finished((su, msg) => {
        if (su) {
          ...
          for (let i in self.MapDetailList) {
            ...
            detailInfo.typePrimaryId = self.MapDetailList[i].typePrimaryId;
            ...
            detailInfo.instanceType = self.MapDetailList[i].instanceType;
            detailInfo.itemImageId = self.MapDetailList[i].itemImageId;
